...
public class CodeStyle_kotlin
{
  ...
  public static CodeStyle.Grammar create(@NotNull CodeStyle codeStyle)
  {
    ...
    final CodeStyle.Grammar kotlin = GrammarUtils.extend(
    ...
    final CodeStyle.Grammar interpolationInside;
    {
      ...
      final List<CodeStyle.Token> tokens = new ArrayList<>(kotlin.tokens().size() + 1);
    ...
    final CodeStyle.Token string = GrammarUtils.findToken(kotlin, "string");
    final CodeStyle.Token rawString = GrammarUtils.findToken(kotlin, "raw-string");

    if (string != null
      && rawString != null)
    {

      final CodeStyle.Pattern stringPattern = string.patterns().get(0);
      final CodeStyle.Pattern rawStringPattern = rawString.patterns().get(0);
